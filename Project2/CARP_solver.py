...
class Graph:  # 无向图
    def __init__(self, edges, vertices):  # labels为标点名称
        self.Edges = edges  # 字典表示边 {(a,b):int}, a<=b
        self.VertexNum = vertices + 1  # 默认1开始
        self.Vertex = [i for i in range(1, self.VertexNum)]
...
def op2(state):
    ...
    for i in range(res[0], res[1]):
        new_state[i] = state[res[0] + res[1] - i - 1]
        new_state[i] = (new_state[i][1], new_state[i][0])
    return new_state
...
def SimulatedAnnealing(initial, schedule, halt, log_interval=200):
    ...
    while not halt(T):
        ...
        # if t % log_interval == 0:
        #     print(f"step {t}: T={T}, current_value={value}")
        t += 1
        T = schedule(t)
    # _, v = TotalCost(state)
