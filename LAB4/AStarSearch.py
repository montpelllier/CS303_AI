...
def AStarSearch(Graph,start,end,distances):
    ...
    while True:
        print(queue)
        yield [nd[1] for nd in queue]  # yield queue whenever before an element popped out from the queue
        pop = heapq.heappop(queue)
        if pop[1] != 'B':
            for nd in Graph[pop[1]]:
                heapq.heappush( queue, (nd[1]+distances[nd[0]]+pop[0]-distances[pop[1]], nd[0]) )
        else:
            yield [nd[1] for nd in queue]
            break
