...
class AI(object):
    ...
    def __init__(self, chessboard_size, color, time_out):
        ...
        self.starA = [(0, 1), (1, 0), (1, chessboard_size - 1), (0, chessboard_size - 2),
                      (chessboard_size - 2, 0), (chessboard_size - 1, 1),
                      (chessboard_size - 2, chessboard_size - 1), (chessboard_size - 1, chessboard_size - 2),
                      ]
        self.starB = [(1, 1), (1, chessboard_size - 2), (chessboard_size - 2, 1),
    ...
    def go(self, chessboard):
        ...
        self.candidate_list = self.sortActions(self.candidate_list)
        ...
        for move in self.candidate_list:
            ...
            if tempScore > bestScore:
                if move in self.corner and bestMove is not None and tempScore-bestScore < 250:
                    pass
                else:
    ...
    def sortActions(self, actions):
        cornerMove, otherMove, starAMove, starBMove = [], [], [], []
        for move in actions:
            if move in self.corner:
                cornerMove.append(move)
            elif move in self.starA:
                starAMove.append(move)
            elif move in self.starB:
                starBMove.append(move)
            else:
                otherMove.append(move)
        return starAMove + starBMove + otherMove + cornerMove
    ...
    def maxValue(self, chessboard, depth, color, alpha, beta):
        nextDepth = depth - 1
        if self.isTerminal(chessboard) or depth == 0 or time.time() > self.time_out - 0.27:
            return self.evaluate(chessboard, color)
        if time.time() > self.time_out - 1.0 and nextDepth > 1:
            nextDepth = 1
    ...
    def minValue(self, chessboard, depth, color, alpha, beta):
        nextDepth = depth - 1
        if self.isTerminal(chessboard) or time.time() > self.time_out - 0.27 or depth == 0:
            return self.evaluate(chessboard, color)
        if time.time() > self.time_out - 1.0 and nextDepth > 1:
            nextDepth = 1
    ...
    def evaluate(self, chessboard, color):  # 估值函数 少子方胜
        ...
        for i in range(4):
            if chessboard[self.starA[2 * i]] == chessboard[self.starA[2 * i + 1]] == chessboard[self.starB[i]]:
                #weightMap[self.starB[i]] += 200
                if chessboard[self.starB[i]] == self.color:
                    weight += 500
                elif chessboard[self.starB[i]] == -self.color:
                    weight -= 500
            # elif chessboard[self.starA[2 * i]] + chessboard[self.starA[2 * i + 1]] == -self.color:
            #     weightMap[self.starB[i]] -= 250
            # elif chessboard[self.starA[2*i]] == chessboard[self.starA[2*i+1]] == -self.color:
            #     weightMa
